...
const AnimalDetail = (props) => {
  ...
  const AddInput = ({name}) =>{
    return (
      <>
        <input id={`${name}-addInput`} placeholder="新項目" className="mr-2"></input>
  ...
  return (
    <>
      <div className="animalDetail">
        <div className="box">
          <button className="btn" onClick={openEdit}>編輯</button>
        </div>
        <div className="box">
          <div>生活照：</div>
                  ...
                  detail.foods.length > 0 && detail.foods.map((food,index) => {
                    return (
                      <div className="p-2" key={food}>
                        <span>{index + 1} - </span>
                        <span>{food} </span>
                        <span onClick={()=>delArrayItem('foods', food)}>
                  ...
                  detail.features.length > 0 && detail.features.map((feature, index) => {
                    return (
                      <div className="p-2" key={feature}>
                        <span>{index + 1} - </span>
                        <span>{feature} </span>
                        <span onClick={()=>delArrayItem('features', feature)}>
                          <i class="fas fa-trash-alt"></i>
                        </span>
                      </div>
                    )
        ...
        {
         isEdit && <div className="flex justify-between">
            {
              detail.name && <div className="box">
                <button
